{
  description = "MiniGun - Account Rotation for Cloudflare Free Tier Usage";
  ...
  outputs = { self, nixpkgs, sops-nix, agenix, ... }: let
  ...
  in {
    ...
    in {
      ...
      config = lib.mkIf cfg.enable {
        ...
        # Rotation service
        systemd.services.minigun-rotation = {
        ...
        # Helper script to get current account information
        ...
        # Individual deployment services for each configured service
        systemd.services = lib.mkMerge (map (service: {
    ...
    # Example usage in a standalone module
    lib.minigun = {
      # Helper function to create account rotation configuration
      mkMinigunConfig = { accounts, secretsMethod ? "plain", rotationStrategy ? "weighted", ... }: {
        services.minigun = {
          enable = true;
          inherit accounts secretsMethod rotationStrategy;
        };
      };
      
      # Helper to create deployment scripts
      mkDeployScript = { name, command }: pkgs: pkgs.writeShellScript "deploy-${name}" ''
        #!/usr/bin/env bash
        set -euo pipefail
        
        echo "Deploying ${name} to Cloudflare account: $ACCOUNT_NAME"
        echo "Using credentials: $CF_EMAIL / $CF_ACCOUNT_ID"
    ...
    # Create utility packages for the MiniGun module
    packages = forAllSystems (system: let
      pkgs = nixpkgs.legacyPackages.${system};
    in {
      # Example deployment script for DevDocs MCP
      deployDevDocsMcp = pkgs.writeShellScript "deploy-devdocs-mcp" ''
        #!/usr/bin/env bash
        set -euo pipefail
        
        echo "Deploying DevDocs MCP to Cloudflare account: $ACCOUNT_NAME"
        echo "Using credentials: $CF_EMAIL / $CF_ACCOUNT_ID"
